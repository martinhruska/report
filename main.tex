...
\begin{titlepage}
	...
	\vspace{1cm}
	{\huge\bfseries My Life between Automata and Program Analysis\par}
	\vspace{5cm}
...
\begin{abstract} In the context of shape analysis, counterexample
validation and abstraction refinement are complex and so far not sufficiently
resolved problems. We provide a novel solution to both of these problems in the
context of fully-automated and rather general shape analysis based on forest
automata. Our approach is based on backward symbolic execution on forest
automata, allowing one to derive automata-based interpolants and refine the
automata abstraction used. The approach allows one to distinguish true and
spurious counterexamples and guarantees progress of the abstraction refinement.
We have implemented the approach in the \forester{} tool and present promising
...
not often fully automatical, both facts lead to the higher costs of application of such methods.
On the other hand, there are light-weighted testing methods which can be fully
automatized and can be often used in development by users without a deep expertise (such as Ph.D. in
formal methods).

This thesis approaches the software quality from a point of view of formal verification,
particullary of static analysis.
The developed methods have formal basis and can provide guarantees about correctness
of analysed programs. But they are also fully automatical and can be used by common developers.
...
combining expressivnes, precision and efficiency.
...
to concrete shapes constructed by a program.
Particulary, we use tree automata.
...
by a) improving abstraction (and refinement) techniques to bring a better compromise between precision and efficiency
b) designing a new kind of automata which will be more general (thus more expressive) but still preserving
important theoretical properties such as completness of language inclusion.
...
This section provides summary of the actual state-of-the-art methods for shape analysis
and representation of graphs by automata.
First, the overview of different methods for analysis of progams with dynamic data structures
together with the refinement techniques for abstraction over automata.
Then the existing automata formalisms representing graphs by automata are presented.

\section{Shape Analysis}
\emph{Shape analysis} deals with analysis of possible shapes allocated on heap by an analysed program.
...
In the following text we will view the data structures allocated on a heap as graphs (so called \emph{heap graphs}.
An allocated memory cell on heap correspond to a node of graph and a pointer pointed
from one memory cell to another is represented by edges between nodes.

	  \subsection{Three-valued Logic}
	  This approach uses logic predicates to express relation between nodes of heap graphs.
	  Morever, it introduces the third logic value (\emph{unknown}) to the standard two
	  boolean values. The \emph{unknown} values is used to represent that a relation
	  of values may or may not be in a relation. Such relation is needed when an
	  abstraction is performed. The abstraction baiscally merges some node of heap
	  graphs which creates a summary nodes. When a summary node $s$ consists of merged
	  nodes $u$ and $v$ such that the node $u$ is connected to another node $w$
	  and the node $v$ is not then the relation between $s$ a $w$ is $\emph{unknown}$.

	  Three-valued was one of the first works on shape analysis. The framework is general
	  however it is needed to be parametrized manually to enable representation of different shapes.
	  The precision by refinement was addressed in \cite{tocojsemcetl,uznevimcotobylo}.
	  Scalability was never explored deeper for this approach.
	  
	  \subsection{Separation Logic}

	  \emph{Separation logic} was firstly introduced by Reynolds \cite{reynolds} as an extension
	  ...
	  While the assertions in Hoare logic consists of standard connectives of propositional logi,
	  separation logic extends the framework with the new ones: $emp$ (representing
	  an empty heap), $e \mapsto e'$ (stating that a heap is defined only for address $e'$ having value of the expression $e$
	  and undefined otherwise), $P*Q$ (saying that a heap can be disjoint to the two separated parts $P$ and $Q$,
	  and $P-*Q$ (asserts that a heap $h'$ satisfining $P$ will after union with heap $h$ satisfy $Q$).

	  Separation logic was from the first introduction further extended to concurrent version.
	  The scalability of logic was improved tuned up by second-order biabduction \cite{dino}.
	  Biabduction also enables analysis more complex data structures such as skiplists \cite{sinngapurany}.
	  However expressivness of logic is not powerfull enough to make possible analysis of tree structures.

	  Separation logic has found its way to practice what is illustrated by its application
	  in Facebook \cite{infer}.

	  \subsection{Abstract Regular Model Checking}

	  The \emph{abstract regular model checking} is an automata based method.
	  It employs so called \emph{tree automata} to represent graphs allocated on heap.
	  It uses tree transducers to model semantics of programming language commands for
	  manipulation with heap.
	  ...
	  has to be propagated over the whole model representing heap.

	  Another negative of the approach is lack of a real experimental evaluation.
	  It was only evaluated over models or simplyfied microcode representing an actual program.
	  Therefore the observed results are rather wishfull thinking than a verified solution to a real problem.

	  \subsection{Symbolic Memory Graphs}

	  \emph{Symbolic Memory Graphs} were designed to fit to framework of abstract intepretation as an abstract domain.
	  ...
	  This is confirmed by the repeated wins of the tool in competition on software verification SV-COMP \cite{sv-comp}.

	  The generality is not the strong side of this formalism since it has not been able to
	  model the tree structures yet.

	  \subsection{Bounded Model Checking}

	  \emph{Bounded Model Checking} is not actually a shape analysis since it does not
	  derive shape invariants but it overlaps with the goals of shape analysis.
	  It has capibility to find errors in a given program related to the dynamic structures manipulation.
	  ...
	  All these practical benefits of bounded model checking are documented by success of
	  the tools implementing it in the SV-COMP competition.
	  A trade-off for the practical benefits is unsoundness of approach.
	  It can only find bugs but not prove a program to be correct since the method
	  only search a state space of an analysed program to a certain bound and then stops.
	  There are attempts to get over this bottleneck by k-induction but they are still not mature
	  as the original bounded approach.

	  \subsection{Automatic Refinement of Abstraction for Shape Analysis}
	...
	This section provide an overview of an actual progress on automata used to
	represent (a certain class of) graphs.
	% Graph automata -> bad thing is decidability
	% Graph grammars
	% Bounded  tree width
...
\chapter{Goals of the Thesis}
	...
	Everything is beautifull and superb, bu we improve it.
	Then we will be able to verify everything what we want.
	\section{Bounded Tree Width for Fun and Profit}
	However, everything was not so beautiful at the end.
	And we wanted a more general formalism anyway.
	so here we have a new formalism. nice theoretical properties. closed under union, intersection.
	then we will see what we can we do about it.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Developing Tool for Shape Analysis}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Conclusions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
