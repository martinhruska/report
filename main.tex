...
\begin{titlepage}
	...
	\vspace{1cm}
	{\huge\bfseries Automated Formal Reasoning for Programs with Dynamic Data Structures\par}
	\vspace{5cm}
...
\begin{abstract}
My Ph.D. thesis deals with \emph{shape analysis}\,---\, a field of static analysis
aimed to reasoning about programs with dynamic data structures.
The goal of my thesis is to introduce new techniques for shape analysis
to allow formal verification of a wider spectrum of programs.
The goal consist of designing new underlying formalism for representation of data structures
allocated on heap, refinement of abstraction over forest automata (an existing formalism
used in shape analysis), and enhancing algorithms for manipulation with the various
automata formalisms used in shape analysis.
The report also contains a discussion of related work and plans of the future work.
\end{abstract}
...
is desired to enterprise applications where a hunt for bugs is driven by desire
...
Some are formal and provides high guarantees (such as proving correctness
...
This thesis approaches the software quality from a perspective of formal verification.
The developed methods have formal basis and can provide guarantees about correctness of analysed programs.
They belong to static analysis which works directly with a source
code of an analysed systems.
Since no model of a system is needed it is easier to autmotize verification.
Even if the methods are not fully automatic (such as abstract interpretation)
it is ofter user friendly to parametrize a tool manually to achieve the best
possible results of verfication.
...
The unboudness makes the analysis difficult since we need to model and reason about infinite state spaces.

Our approach to shape analysis is based on automata theory which properties
combine expressiveness, precision and efficiency.
The generality of automata theory leads to expressiveness strong enough to represent most of the practically
used data structures. Moreover, since automata theory is well-established we can build on the existing
results about expressiveness and complexity of various automata variations.
A reasoning about infinite systems is also possible thanks to the abstraction techniques for automata.
The efficiency is achived by many algorithmic heuristics used to make the operations over automata efficient.
The precision can be achieved by automated refinement of abstraction to adjust automata representation
precisly to heap shapes constructed by a program.
Particularly, we built upon tree automata theory which are used to represent data structures
allocated on heap.
Since the heap structures allocated on heap can be viewed as graphs which can be often decomposed
...
\begin{enumerate*}[label=(\alph*)]
	...
	\item designing a new kind of more automata with a higher expressivity but having still complete the important
		operations such as language inclusion, and
	\item to improve an efficiency of existing algorithms for tree automata.
\end{enumerate*}

%TODO maybe specify what will be concrete goals - forest automata and goal automata

The outline of this document is following. The Chapter \ref{ch:state-of-the-art} provides a summary of
...
First, the overview of different methods for analysis of programs with
...
\emph{Shape analysis} deals with analysis of shapes allocated on heap by program.
...
The selectors are presented by the labels over edges.
Formally, a heap graph $G$ is tuple $(V,E,\alpha)$, where $V$ is a set of nodes, $E \subseteq V \times V$ is
a set of edges, and $\alpha: E \rightarrow \selectors$ is a labelling function and $\selectors$ is a set
of selector names.
	  ...
	  \begin{enumerate*}[label=(\alph*)]
	  	...
	  	\item efficiency and scalability\,---\,there exist efficient algorithms for model manipulation
			even when applied to big systems.
	  \end{enumerate*}

	  \subsection{Three-valued Logic}
	  This approach \cite{Sagiv02} uses predicates (as predicate logic) to express relations between nodes of heap graphs.
	  Moreover, it introduces the third logic value (\emph{unknown}) to the standard
	  boolean values \emph{true, false}. The \emph{unknown} values is used when
	  two items from a given universe may or may not be in a relation.
	  ...
	  Three-valued logic was one of the first works on shape analysis. The framework is general
	  however it needs to be parametrized manually by predicates describing possible
	  shapes in an analysed program.
	  The precision of used abstraction by refinement was addressed in \cite{cav05, beyer:lazy_shape_analysis}.
	  The scalability has never been explicitly discussed for this approach but it is (probably)
	  ...
	  and $P-*Q$ (states that a heap $h'$ satisfying the assertion $P$ will after union with heap $h$ satisfy
	  the assertion $Q$).

	  Separation logic has already been extended extended to a concurrent version \cite{brookes16}.
	  The scalability of logic was tuned up by second-order bi-abduction \cite{popl09}.
	  Bi-abduction also enables analysis more complex data structures such as skiplists \cite{cav14}.
	  However the expressiveness of logic is not powerful enough for analysis of general tree structures.
	  Moreover, it is often needed to manually provide inductive predicates describing the heaps shapes even
	  for list structures.
	  ...
	  It employs \emph{tree automata} to represent the reachable configurations of program.
	  It uses tree automata transducers to model semantics of an analysed system.
	  An (height or predicate language) abstraction is used to
	  overapproximate set of reachable configurations.
	  The counterexample-guided refinement is applied to
	  refine abstraction and to validate the found counterexamples when needed.

	  The verification procedure starts with an automaton representing
	  an initial configuration of an analysed program.
	  ...
	  the bad configurations of system is also done in each step.
	  ...
	  The method is general and can represent various graphs allocated on heap.
	  ...
	  It was only evaluated over the models or a simplified microcode representing a program.
	  Therefore the observed results are rather wishful thinking than a verified solution of a real problem.
	  ...
	  They model a state of heap precisely but can perform \emph{widening} to
	  ...
	  The generalization of the domain to trees structures has not yet been yet proposed and due to the lack
	  of the mathematical elegance and generality it does not seem probable that it will
	  be introduced at all.
	  Also the scalability of the method has not been studied it is unlikely
	  that the principles such as bi-abduction will be introduced for SMG soon.
	  ...
	  to a memory manipulation related error.
	  ...
	  The basic approach to refinement of abstraction is \emph{couterexample-guided
	  refinement (CEGAR)} \cite{CEGAR} which works as follows:
	  \begin{enumerate}
		\item Set a defautl abstraction
		\item Perform an analysis of a program
		\item If a counterexample is found
		\begin{enumerate}
			\item Start a validation of counterexample
			\item If it is valid, report an error
			\item Otherwise refine the abstraction and go to the point $2$
		\end{enumerate}
		\item If no counterexample is found, report the program as correct
	  \end{enumerate}
	  Many of the following works are just an application of CEGAR to a specific verification procedure.
	  ...
	  Unlike our work, the approach may, however, fail in recognizing that a given
	...
	The generality of a verification method for shape analysis depends on a choice
	of an underlying formalism enabeling represention of various heap graphs.
	Moreover, some data structures are defined with some special relations
	between nodes or set of nodes.
	Therefore we need a formalism allowing also representation of these constraints over graphs.

	In the field of formal logic (a useless branch of mathematics), a natural choice
	is monadic second order logic.
	It allows a quantification over sets so when the graphs are chosen as an universe
	...
	the crucial properties such as emptiness of automata language.
	That is result obtained by \cite{thomas91, reiter15} where the designed automata
	suffers from the undecidability of the crucial properties.
	...
	This is difference compared to word or tree automata where an automaton runs over
	...
	In \cite{reiter15}, there is a special automaton for each node of graph, the automata
	communicates in rounds and the computation continues until all automata reach their stable state.
	...
	A decision procedure is then implemented by encoding the formula to tree automaton
	and checking its emptiness.
	Unfortunately, the tree automaton has exponential number of states
	compared to the size of the formula what makes a manipulation with it inefficient.

	Another approach to the definition of graph automata is from an algebraic point of view.
	In the following work \cite{blume:2012}, the automata were defined using concepts
	of \emph{cospans} from category theory.
	Intuitively, the automata have as symbols basic graph operations which can create
	an arbitrary graph with bounded tree width.
	Such definition implies that word and graph automata are basically the same class
	because the basic operations can be viewed just as a standard letters from an alphabeth
	of word automata.
	However, the authors have not found out an efficient automatic derivation of their model
	from a system description.

	Grammars are a standard counterpart to automata in formal language theory.
	E.g. context-free grammars characterize the class of context-free languages just
	as pushdown automata.
	The paper \cite{brandenburg05} explores relation between automata working over
	graphs and shows that they describe the same class of graph languages as
	the NCE graph grammars.
	
	The natural comparison is also to the separation logic which was originally designed
	...
	which can be generated by so called \emph{tree-like grammars (TLG)}.
	Both formalisms are meant to describe tree-like data structures (i.e. data structures
	decomposable to trees).
	Further, TLG formula can be transformed to $MSO_2$ logic which is a variant of MSO
	...
	The example is emptiness of automata with auxiliary storage in \cite{popl11}.

\chapter{Goals of the Thesis}
\label{ch:goals}
	This section summarizes goals of this thesis and provides their brief description.
	The general goals is to improve the state-of-the-art of shape analysis.
	We will develop the particular methods to achieve the goal which may yield a new
	contributions also to the other fields than shape analysis.

	There three main sub-goals and each of them should be fulfilled during the work on the thesis, however
	...
	On the other hand, the overapproximation lead to a detection of spurious counterexamples,
	i.e., the counterexamples not presented in a system under verification.

	Reporting found bugs in a program is possible only when the counterexample is validated.
	Therefore one of the goals is to design so called \emph{backward run} which is a method
	...
	of abstraction for forest automata will be based on CEGAR \cite{CEGAR} and will
	be inspired of the application of the CEGAR to ARTMC.
	However, it will be still needed to design the new features for forest automata
	such as predicate language abstraction or intersection to fulfill this goal.
	
	As follows from the previous text, these goals should enable a reliable reporting of the found bugs
	...
	following chapter) there are data structures (such as B+ tree) which would need more
	...
	It makes modelling of the quite straightforward practical operations (such as memset)
	...
	They should be theoretically elegant, have decidable important properties (e.g., language inclusion),
	...
	The abstraction of heap representation will not be higher than necessary
	(i.e., abstraction only for sake of theory as it is usual in the field of formal methods)
	...
	In this work, we will focus on the efficiency needed in formal verification application
	of automata. The needed efficient operations are, as in the case of graph automata,
	language inclusion checking, reduction of automata states, or product of two automata.
	
	For example, the inclusion operation is in the PSPACE complexity class
	for tree automata which means that no general efficient algorithm exists.
	...
	to avoid state exposition in many practical cases.

	The goal of this thesis is to design new and to improve existing
	algorithms for the (not only) mentioned operations.
	An improvement in the efficiency could help to deal with state explosion and so bring
	the existing methods based on tree automata to applications in the more difficult cases.
...
The report defined the thesis goals in the field of shape analysis.
They are following:
	  \begin{enumerate*}[label=(\alph*)]
		\item design counterexample validation and predicate language abstraction
			with refinement for forest automata,
		\item design automata for accepting graphs with bounded tree width,
		\item design new efficient algorithms for (tree) automata manipulation.
	  \end{enumerate*}
The report also provided with the description of the related work containing
the state-of-the-art techniques in the shape analysis.
...
Therefore the Forester tool implementing forest automata based shaped
...
The remaining work consists of the second and the third goal.
