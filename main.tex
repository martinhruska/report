...
The flexibility comes also from the automata abstractions techniques used that are not in advance fixed for a particular
...
of selector names.
	  ...
	  Scalability of the method has not been addressed.
	  ...
	  the empty heap), $P*Q$ (saying that a heap can be splitted to two separated parts where one satisfies $P$ and the second $Q$),
	  ...
	  One of such methods is \emph{bounded model checking (BMC)} which achieved many achievments
	...
	may lead to the explicit determinisation of an automaton which may cause an exponential
	...
	all of the mentioned techniques do not continue in building the product automaton from
	some product states which are not needed to be further explored to confirm or disprove that inclusion holds.
...
\label{ch:goals}
	...
	The following section summarizes how we want to improvement the current state of shape analysis.
	...
	Symbolic automata have logical formulas in the predicate symbols instead of symbols.
	It enables an efficient representation of the large, or even infinite alphabeths still having
	...
	for symbolic automata which will be needed for further reasearch.
...
Particularly, we aim to development the following advancement:
	  \begin{enumerate*}[label=(\alph*)]
