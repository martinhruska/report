...
and proposing a new formalism capable of analysis of program on which the current analyses fail.
...
One of eternal quests of computer science is to achieve software of
...
of a program using a proof assistent or programming in a language
with an expressive and safe type system).
Moreover, they require highly specialized developers and are often not fully automatic.
Both of these facts lead to higher costs of such techniques.
Then there are techniques with formal bases and with higher or even full automization.
They provide some formal guarantees of code quality and can be run by a developer.
As a trade-off, they have problems with scaling to large software systems a often
produce false positives, i.e. detect an error which is not in fact in an analysed system.
This class of methods includes different model checking and static analysis techniques.
On the other end scales, there are light-weight testing methods which can be fully
automized and can often be used in development by users without a deep expertise
...
Particulary, they belong to static analysis which works directly with a source
code of an analysed system.
Since no model of a system is needed, it is easier to automize verification.
Static analysis algorithms also does not rely on help from a user as theorem
proving which sometines needs an advice to choose the right proof rule.
...
In static analysis, it is common that the particular methods focuse on particular
...
which are basically tuples of tree automata (will be further described in Chapter \ref{ch:vmcai}).
Forest automata accept tuples of (interconnected) trees as members of their languages.
When the trees in a tuple are connected, we obtain one of heap graph represented
...
The flexibility comes from the automata abstraction used that are not in advance fixed for a particular
class of data structures.
The abstraction can learn a suitable way of representing a given structure automatically for many usefull cases.
...
We will describe the most sucessfull methods and those based on automata.
Then refinement techniques for abstraction over various formalisms are summarized.
Finally, existing work on automata for graph languages is presented.

\section{Shape Analysis}
\emph{Shape analysis} deals with analysis of shapes allocated on heap by a program.
...
The selectors are presented by the labels of the edges.
...
of selector names.
	  ...
	  The method has not been extended to scale to large software systems.
	  ...
	  the empty heap), $e \mapsto e'$ (an address defined by the expression $e$ is mapped to a value defined
	  by the expression $e'$, and the rest of the heap is empty, i.e. $*emp$ is implicit),
	  $P*Q$ (saying that a heap can be splitted to two separated parts where one satisfies $P$ and the second $Q$),
	  ...
	  abstract-interepretation-based approaches associated with Space Invader \cite{InvaderCAV08}.
	  Separation logic has been extended to a concurrent version \cite{brookes16}.
	  The scalability of the approach based on separation logic was significantlly improved by bi-abduction \cite{popl09}.
	  ...
	  The similiar principles as second-orderd bi-abduction enabeling analysis of complex data structures
	  ...
	  However, the domain has not been yet generalized to the trees structures.
	  The scalability of the method also has not been studied and but it can
	  employ principles of bi-abduction in the same fashion
	  as separation logic-based approaches did.

	  \subsection{Abstract Regular Model Checking}
	  \label{subsection:armc}
	  The \emph{abstract regular model checking} \cite{artmc12} is an automata based method for verification of parametrised and infinite state systems.
	  ...
	  A (e.g., height or predicate language) abstraction is used to
	  ...
	  heap in one automaton, even a small change in one part of a heap may have to
	  ...
	  tree automata, so called forest automata, which localise a change in one part of heap
	  ...
	  fully automatical and was able to verify structures such as skiplists.

	  The approach was applied to verification of the C programs with dynamic data structures in
	  work on forest automata-based shape analysis \cite{forester12}.
	  The tool Forester implementing the methods is able to verify complex data structures
	  such as various tree and skiplists of the second and thrid level \cite{boxes13, holik:2017} which
	  ...
	  assumed that a used verification procuder will be sound.
	  ...
	  One of such methods is \emph{bounded model checking (BMC)} which achieved many
	  in the various editions of the SV-COMP competition \cite{www:svcomp}.

	  An exploration of state stace is limited up to some bound in BMC.
	  ...
	  BMC systematically explores the state space of (a model of) an analysed systems up
	  ...
	  A model of an analysed system can be automatically derived from an analysed program.
	  Once an unbounded loop occurs in program it is explored (unwrapped)
	  to a certain \emph{bound} (which determines the name of the method).
	  This approach to nondeterminism leads to unsoundness of the approach.

	  The approach may be fully automated and can be easily combined with other domains of 
	  interest in program analysis, e.g., analysis of float values.
	...
	In the field of formal logic (a useless branch of mathematics),
	a natural choice is monadic second order logic on graphs.
	...
	The undecidability does not neccessarily imply that formalism is not usable (since sufficient
	heuristics may exists) it is better to target decadability, or at least a formalism
