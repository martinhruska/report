...
enabling verification of more complex software systems.
The goal consists of designing a new underlying formalism for data structures representation,
refinement of abstraction over forest automata (an existing formalism for shape analysis),
and enhancing algorithms for manipulation with the various automata used in shape analysis.
The report also contains a discussion of a related work and the plans of the future work.
\end{abstract}
...
not often fully automatic, the both facts lead to the higher costs of their application.
...
combining the upsides of the both worlds.
...
a user can often easily parametrize a tool manually to achieve the best
...
and verifies memory manipulation related properties such as absence of memory leak, invalid dereference or invalid free.
...
It combines expressiveness, flexibility and efficiency.
...
The flexibility can be achieved by automated refinement of abstraction to adjust automata representation
precisely to heap shapes constructed by a program.
The efficiency comes from many algorithmic heuristics developed to deal with state explosion
in the crucial operations over automata.

Particularly, we built on tree automata theory which are used to represent data structures
allocated on heap.
Since the heap structures allocated on heap can be viewed as graphs decomposable
...
an actual progress on abstraction refinement for forest automata.
...
of selector names.
	  ...
	  but it needs to be parametrized manually by predicates describing possible
	  ...
	  The method has been (probably) used in a technology of GrammaTech company for analysis of industry code.
	  ...
	  The $P$ assertion should be satisfied before an execution of $C$ and the $Q$ assertion after the execution.
	  Another possible formulation is that predicates from set $P$ are transformed to set $Q$
	  with respect to semantics of $C$.

	  While the assertions in Hoare logic consists of standard connectives and symbols of predicate logic,
	  ...
	  by expression $e'$ and the rest of the heap is undefined),
	  $P*Q$ (saying that a heap can be disjointed to the two separated parts where one satisfies $P$ and the second $Q$),
	  and $P-*Q$ (states that heap $h'$ satisfying the assertion $P$ will after union with heap $h$ satisfy
	  the assertion $Q$).
	  ...
	  and \emph{tree automata transducers} to model semantics of an analysed system.
	  ...
	  The possible configurations of heaps are derived fully automatically and there
	  ...
	  Therefore a verification of software using the method is rather a wishful thinking than a verified solution.
	  ...
	  of the mathematical elegance and beauty it does not seem probable that it will
	  be introduced at all.
	  The scalability of the method also has not been studied and it is unlikely
	  ...
	  may be possible allocated on heap in a given state of program
	  ...
	  This approach to nondeterminism leads to unsoundness of the approach.

	  The approach is fully automated and can be easily combined with other domains of 
	  interest in program analysis,
	  ...
	  The basic approach to refinement of abstraction is \emph{counterexample-guided
	  refinement (CEGAR)} \cite{CEGAR} which works as follows:
	  \begin{enumerate}
		\item Set a default abstraction
	...
	of an underlying formalism enabling representation of various heap graphs.
	Moreover, some data structures are defined with special relations
	between nodes or set of nodes (e.g., red-black trees have red and black nodes with
	the special rules how they can alternate).
	Therefore the formalism should make possible representation of these relations over graphs.
	...
	suffers from the undecidability of some properties.
	...
	Contrarily, a graph automaton has to remember its path through the graph.
	...
	When the operations are concatenated we obtain encoded graph.
	...
	The both works, comparing graph automata to grammars and separation logic,
	explores expressive power of automata and its relation to the other formalisms
	but does not concern about the efficient algorithms and their properties
	so they are not suitable for applications in shape analysis.
...
\label{ch:goals}
	The following section summarizes goals of this thesis and provides their brief description.
	...
	There are the three main sub-goals and each of them should be fulfilled during the work on the thesis, however
	the directions and goals of the research may change according to the actual development in the field.
	...
	such as predicate language abstraction or automata intersection to fulfill this goal.
	...
	The generality will not be the only advantage of the new automata.
	...
	It makes modelling of the quite straightforward practical operations (such as the C function \emph{memset})
	cumbersome and not easy to design and implement for hierarchical forest automata.

	Therefore new automata should address the both problems.
	...
	The abstraction of heap will not be higher than necessary
	...
	Although the mentioned properties will the implementation of the new automata easier
	the main scientific contribution of the thesis will
	...
	which means that no efficient algorithm exists.
	...
	Fortunately, the different heuristic approaches based on
	antichains \cite{ac:2006}, simulations \cite{henzinger:1995}, or their combinations \cite{tacas10}
	were proposed to avoid a state explosion in many practical cases.

	The goal of this thesis is to design new and to improve existing
	algorithms for (not only) the mentioned operations.
	An improvement in the efficiency could help to deal with state explosion and bring
	the existing methods based on tree automata to analysis of more complex software.
...
This report defined the following goals of the thesis:
	  \begin{enumerate*}[label=(\alph*)]
...
description was given based on the paper \cite{holik:2017}.

The authors are not afraid of an experimental evaluation providing
a hard and clear evaluation of the developed methods.
