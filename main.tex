...
of selector names.
	...
	relies on the efficiency of operations used for automata manipulation.
	Unlike in the other fields of computer science, formal verification methods
	...
	would lead to the explicit determinisation of an automaton which may cause an exponentional
	blow up of the number of the automaton states.
	Therefore heuristic techniques preventing the expliciti determinization were introduced \cite{ac:2006, bouchi2013, tacas10}.
	Since language inclusion is EXPTIME complete it is not possible to implement a general efficient
	algoritmh but the mentioned heuristic works well for many verification tasks.

	Another important factor for verification is size of the automaton.
	We would like to work with a minimal automaton which would increase
	efficiency of the verification procedure.
	Unfortunatelly, the textbook approach is consists of determinisation of
	...
	The simulation relation can be computed in poylnomial time \cite{ilie:nfa, fosc95}.
	It basically says that if one state is simulated by another state the all string (or trees)
...
\label{ch:goals}
	The following section summarizes goals of the proposed thesis and provides their brief description.
	The goal is to improve the state of the art of shape analysis.
	Moreover, some of the proposed methods that we intended to develop to achieve this goal
	...
	Each of subgoals should be fulfilled during the work on the thesis, however,
	the directions and goals of the research may change to some degree according
	...
	i.e., counterexamples not present in the system under verification.
	...
	In fact, this sub-goal has already been fullfilled and published in \cite{holik:2017}
	and will be described deeply in Chapter \ref{ch:vmcai}.
	The approach is based on CEGAR \cite{CEGAR} and will be inspired of the application of the CEGAR to ARTMC.
	...
	many existing approaches (e.g. abstract regular tree model checking based on forest automata)
	relies on tree automata.
	Therefore efficient algorithms for automata manipulation are needed.
	In this work, we will focus on the efficiency needed in formal verification based on automata.
	The needed efficient operations are, as in the case of graph automata,
	language inclusion checking, reduction of automata states, or product of two automata.
	
	For example, the inclusion operation is EXPTIME-complete complexity class
	which means that no efficient algorithm exists in general.
	A naive algorithm for this operations would lead to an implicit determinisation
	of the checked automata which would cause an exponential grow of the number of states.
	Fortunately, different heuristic approaches based on
	antichains \cite{ac:2006}, simulations \cite{henzinger:1995}, or their combinations \cite{tacas10}
	were proposed to avoid the state explosion in many practical cases.

	The goal of this thesis is to design new and to improve the existing
	algorithms for (not only) the mentioned operations.
	An improvement in the efficiency could help to deal with the state explosion and bring
	the existing methods based on tree automata to analysis of more complex software.

	Particulary, we will focuse on generalization of computation of congruence closure used
	...
	in the same manner as it helped for classical word automata in \cite{bonchi2013}.
	We implemented the congruence approach for word automata in the VATA library \cite{libvata}
	...
	There is currently intesinve research in this field and we suppose
	...
	may be theoretically tempting but it leads to not elegant and spaghetti implementation of
	...
	and implement for hierarchical forest automata.
	...
	the main scientific contribution of the thesis will focus on efficient algorithms for the automata.
	...
	how they go through a processed graph.
	...
	However, although the symbols will have the inner semantics it should not effect manipulation with automata.
	Therefore the complexity of operations with automata will be the same as for classical tree automata
	and we will use the existing heuristics for the crutcial operations such as language inclusion.
...
Particulary, we aim to developemnt the following advancement:
	  \begin{enumerate*}[label=(\alph*)]
		\item design techniques for counterexample validation and refinement
			of predicate language abstraction over forest automata,
		\item design new automata for accepting graphs with bounded tree width,
...
The results achieved in \cite{forester12} haven been implemented (in several years)
in the Forester tool and helps in participation in the last three editions
of SV-COMP \cite{www:svcomp} in the memory safety related categories.
